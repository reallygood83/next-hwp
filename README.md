...
- HWP/HWPX/TXT/MD 문서를 페이지형 HTML 뷰어로 미리보기
- 텍스트 직접 붙여넣기 지원
...
HWPX는 zip 내부 XML 텍스트를 추출합니다. HWP 바이너리는 OLE Compound File의 `BodyText/Section*` 스트림에서 `PARA_TEXT` 레코드를 추출해 텍스트 중심으로 표시합니다. 표, 그림, 각주, 정밀 페이지 레이아웃은 아직 완전 렌더링이 아니므로 운영 수준의 원문 재현은 rhwp/WASM 또는 서버-side 변환기를 붙이는 단계가 필요합니다. 서버 API는 문서 원문과 API key를 로그에 남기지 않도록 작성되어 있습니다.
