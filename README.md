...
- 공유 페이지에서 HTML, MP3, 선택형 원문 PDF와 선택형 HWP/HWPX 뷰어 제공
- 변환된 음성 포함 HTML을 서버 저장공간에 저장하고 `/s/{id}` 공유 링크 생성
- 공유 저장 파일은 Firebase Storage 규칙 기준 전체 15MB 이하로 제한
- Firebase Google 로그인으로 앱 사용 보호
- 교사/공무원 대상 랜딩페이지 제공
- 다문화 가정 학생·학부모 안내와 민원 답변을 다국어 음성 브리핑으로 배포하는 활용 시나리오 포함
- Firebase Storage에 공유 오디오, 선택형 원문 PDF, 선택형 원본 HWP/HWPX 뷰어 파일 저장, Firestore에 공유 페이지용 구조화 데이터 저장
...
공유 링크는 Firestore의 구조화된 브리핑 데이터와 Firebase Storage의 오디오 파일, 선택형 원문 PDF, 선택형 HWP/HWPX 원본 뷰어 파일을 조합해 `/s/{id}` 서버 템플릿으로 제공합니다. 공유 페이지는 임의 HTML을 그대로 실행하지 않고 CSP를 적용합니다. HWP 뷰어 공유는 원본 파일을 공개 링크에서 읽을 수 있게 하므로 개인정보 문서에서는 기본 비활성 상태로 두는 것을 권장합니다.

서버 비용과 유지 비용 선택지는 [Deployment and Cost Notes](docs/deployment-costs.md)를 참고하세요. 공개 운영에서는 사용자별 저장 개수 제한, 파일당 15MB 제한, 만료 정책, 로컬 백업 안내를 함께 유지하는 편이 안전합니다.
